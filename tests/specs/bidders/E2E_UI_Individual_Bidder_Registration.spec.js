...
productdata.forEach((bidderEnv) => {
    test(`${bidderEnv.product} in ${bidderEnv.environment}`, async ({ page }) => {
        const logger = new Logger(`Creating Individual Bidder Registration for ${bidderEnv.product} in ${bidderEnv.environment}`)
        logger.logInfo(`Creating Individual Bidder Registration for ${bidderEnv.product} in ${bidderEnv.environment}`)
