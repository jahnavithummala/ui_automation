...
const scenarioName = bidderScenarios.scenario3;
...
filteredProductData.forEach((bidderEnv) => {
    test.describe(`Scenario: ${scenarioName} - ${bidderEnv.product}`, () => {
        test(`Dealer Bidder Registration ${bidderEnv.product} - ${environment}`, async ({ page, bidder_registration, create_account }) => {
            ...
            logger.logInfo('Creating Dealer Bidder Registration')
