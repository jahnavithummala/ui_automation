...
const scenarioName = scenarioNames.scenario8;
...
vehicleData.forEach((vehicle) => {
    test.describe(`Scenario: ${scenarioName}`, () => {
        test(`${vehicle.year} ${vehicle.make} ${vehicle.model}`, async ({ page, create_account, addvin, vehicledetails, titledocuments, accountdashboard }) => {
            await allure.epic(`Consignment - ${environment}`)
            await allure.feature('Consign and AddAllTypesOfAddressesToAnAccount')
            ...
            const uploadVehicleDocs = false;
            if (uploadVehicleDocs) {
            ...
            await allure.step('Click Next documents skipped', async () => {
                logger.logStep('Clicking Next - vehicle documents skipped')
                await addvin.clickNext()
            })
            ...
            await allure.step('Add Multiple Addresses for Account', async () => {
                await accountdashboard.addAddressTypesToAccount()
                await page.waitForTimeout(2000)
                const addressSS = await page.screenshot({ fullPage: true });
                await allure.attachment("Added Address Types", addressSS, "image/png");
                logger.logInfo('Added All Address Types for an Account')
            })
        })
    })
})
