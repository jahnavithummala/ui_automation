...
const scenarioName = scenarioNames.scenario3;
...
vehicleData.forEach((vehicle) => {
    test.describe(`Scenario: ${scenarioName}`, () => {
        test(`${vehicle.year} ${vehicle.make} ${vehicle.model}`, async () => {
            ...
            logger.logInfo('Creating Trust Consignment')
            ...
            await allure.step('Add TitleTo as Trust', async () => {
                logger.logStep(`Adding titled to as - ${chalk.bold.italic.green('Trust')}`)
                await titledocuments.addTrustTitledTo(accData.firstName, accData.lastName)
            })
            ...
            logger.logInfo('Trust Consignment created successfully')
